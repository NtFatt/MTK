...
CREATE TABLE `order_items` (
  ...
  `kitchen_status` varchar(30) NOT NULL DEFAULT 'NEW',
  `kitchen_received_at` datetime DEFAULT NULL,
  `kitchen_preparing_at` datetime DEFAULT NULL,
  `kitchen_ready_at` datetime DEFAULT NULL,
  `created_at` timestamp NOT NULL DEFAULT CURRENT_TIMESTAMP,
  `updated_at` timestamp NOT NULL DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP,
  PRIMARY KEY (`order_item_id`),
  KEY `fk_oi_item` (`item_id`),
  KEY `idx_oi_order` (`order_id`),
  KEY `idx_oi_order_kitchen_status` (`order_id`,`kitchen_status`,`created_at`),
