...
export function InternalPosMenuPage() {
    ...
    useEffect(() => {
        ...
        (async () => {
            ...
            if (ck) setSession({ sessionKey, cartKey: ck });
            ...
            // cart response của bạn có cart.branchId => lấy cái này làm chuẩn
            const bid =
                detail?.cart?.branchId ??
                fallbackBranchId ??
                null;
            ...
            if (bid != null) {
                ...
                const res = await apiFetch<any>(
                    `/menu/items?branchId=${encodeURIComponent(String(bid))}&limit=200`
                );
    ...
    }, [sessionKey, setSession, fallbackBranchId]);
    ...
    async function setQty(itemId: string, qty: number) {
        ...
        try {
            ...
            setSession({ sessionKey, cartKey: ck });
        } finally {
            setBusy(false);
        }
    }
                                        ...
                                        <button
                                            ...
                                            onClick={() => {
                                                ...
                                                if (remain != null) {
                                                    ...
                                                    // sắp hết: chỉ cảnh báo (vẫn cho thêm)
                                                    if (remain <= 5) {
                                                        setWarn(`Cảnh báo: "${it.name ?? it.id}" sắp hết, còn ${remain}.`);
                                                    }
                                                }
                            ...
                            void (async () => {
                                ...
                                try {
                                    ...
                                    if (code === "OUT_OF_STOCK") {
                                        setCheckoutError("Không đủ tồn kho để đặt món. Vui lòng giảm số lượng các món sắp hết/hết hàng.");
                                    } else {
