...
export function InternalMenuRecipesPage() {
  ...
  const ingredientOptions = inventoryItemsQuery.data ?? [];
  const items = useMemo(() => {
  ...
  return (
    <div className="mx-auto max-w-7xl space-y-6">
      <div className="flex flex-col justify-between gap-3 md:flex-row md:items-center">
        ...
        <Button type="button" onClick={onSave} disabled={!selectedId || saveMutation.isPending}>
                  ...
                  <table className="min-w-full text-sm">
                    ...
                    <tbody>
                      {lines.map((line, idx) => {
                        ...
                        return (
                          <tr key={`${selected?.id ?? "recipe"}-${idx}`} className="border-t">
